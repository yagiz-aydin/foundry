...
function assertFoundryAvailable() {
  try {
    if (process.platform === "win32") {
      child_process.execSync("where foundry");
    } else {
      child_process.execSync("which foundry");
    }
  } catch (error) {
...
async function getServiceUrl() {
  try {
    const stdout = await new Promise((resolve, reject) => {
      child_process.exec("foundry service status", (err, stdout2) => {
        if (err) {
          reject(err);
          return;
        }
...
async function startService() {
  ...
  child_process.exec("foundry service start", (err) => {
    if (err) {
...
class FoundryLocalManager extends base.FoundryLocalManager {
  /**
...
exports.DeviceType = base.DeviceType;
exports.ExecutionProvider = base.ExecutionProvider;
exports.FoundryLocalManager = FoundryLocalManager;
